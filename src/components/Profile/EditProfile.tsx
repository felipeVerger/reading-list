...
const EditProfile: FC<Props> = ({ user, setActiveComponent }) => {
  const [avatar, setAvatar] = useState<
    null | Avatar | SanityImageAssetDocument
  >(null);
      ...
      <label htmlFor="file" className="self-center cursor-pointer">
        <div className="relative w-32 h-32 rounded-full border-none overflow-hidden">
          {loading.isAvatarLoading ? (
            ...
            <img src={urlFor(avatar)} alt="new avatar" className="w-full h-full object-cover"/>
          ) : (
            <img src={urlFor(user.image)} alt="avatar" className="w-full h-full object-cover"/>
          )}
          ...
          <>
            {loading.isHomeBgLoading ? (
              ...
              <div className="w-full flex flex-col justify-center items-center gap-2">
                <IconLibrary.errorImage />
                <p className="text-sm text-red-400">
                  Wrong image type, the only supported types are: png, svg,
                  jpeg, gif and tiff
                </p>
              </div>
            ) : homeBg ? (
